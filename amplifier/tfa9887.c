...
#ifdef TFA9887_DUMP_STACK_DEBUG_ENABLE
...
static int tfa9887_hw_power(struct tfa9887_amp_t *amp, bool on)
{
    ...
    //zekken
    reg_val[0]=0;
    reg_val[1]=(unsigned int)on;
    if ((rc = ioctl(amp->fd, TFA9887_SET_SPKAMP, &reg_val)) != 0) {
	  ALOGE("%s: ioctl TFA9887_SET_SPKAMP failed, rc = %d", __func__, -errno);
	  goto power_err;
    }
    amp->is_on = on;
...
static int tfa9887_select_input(struct tfa9887_amp_t *amp, int input)
{
    ...
    switch (input) {
	case 0: /* I2S input 1 Left */
            value |= 0x00;
            break;
        case 1: /* I2S input 1 Right */
            value |= 0x40;
            break;
        case 2: /* Output Coolflux */
            value |= 0x80;
            break;
	case 3: /* Output Coolflux */
	    value |= 0xC0;
	    break;
        default:
...
static int tfa9887_select_output_data(struct tfa9887_amp_t *amp, unsigned char data)
{
    int rc;
    uint16_t value;
    ...
    if (rc != 0) {
        ALOGE("%s: Unable to read from TFA9887_I2S_CONTROL", __func__);
        goto select_output_err;
    }

    // clear the 2 bits first
    value &= ~(0x3 << TFA9887_I2SCTRL_DATA_OUT_SHIFT);

    switch(data) {
        case TFA9887_I2SCTRL_LOOPBACK_DEFAULT:
	    value |= 0x00;
	    break;
	case TFA9887_I2SCTRL_LOOPBACK_DATA1:
	    value |= 0x01;
	    break;
	case TFA9887_I2SCTRL_LOOPBACK_DATA2:
	    value |= 0x02;
	    break;
	case TFA9887_I2SCTRL_LOOPBACK_DATA3:
	    value |= 0x03;
	    break;
        default:
            ALOGW("%s: Invalid output line requested: %d", __func__, data);
            rc = -1;
            goto select_output_err;
    }
...
static int tfa9887_enable_dataout(struct tfa9887_amp_t *amp, bool enable )
{
    int rc;
    uint16_t value;
...
static int tfa9887_lock(struct tfa9887_amp_t *amp, bool lock)
{
    ...
    if (!amp) {
	ALOGE("%s: No amp device\n",__func__);
        return -ENODEV;
    }

    if (amp->fd < 0) {
	ALOGE("%s: illegal amp device fd num %d\n", __func__, amp->fd);
        return -ENODEV;
    }
...
static int check_sys_control(struct tfa9887_amp_t *amp)
{
    int rc;
    uint16_t value;

    if( !amp ) {
        ALOGE("%s: Amp device not found\n", __func__);
        return -ENODEV;
    }    
...
static int change_path_offload(struct tfa9887_amp_t *amp)
{
    int rc;
    uint16_t value;
...
/* Public functions */
int tfa9887_check_status(void)
{
    struct tfa9887_amp_t *amp = NULL;
    int i;

    if(!amps) {
        ALOGE("%s: TFA9887 is not open\n", __func__);
	return 0;
    }

    for(i=0; i<AMP_MAX; i++) {
        amp = &amps[i];
        check_sys_control(amp);
    }

    return 0;
}
int tfa9887_change_path(bool offload)
{
   struct tfa9887_amp_t *amp = NULL;
   int rc, i;
...
int tfa9887_power(bool on, bool offload)
{
    int rc, i;
    struct tfa9887_amp_t *amp = NULL;
    ...
    for (i = 0; i < AMP_MAX; i++) {
        ...
        rc = tfa9887_enable_dataout(amp, tfa9887_amp_enable );
	if(rc) {
