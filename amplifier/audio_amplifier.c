...
static int amp_output_stream_start(amplifier_device_t *device, struct audio_stream_out *stream, bool offload)
{
    amp_device_t *dev = (amp_device_t *) device;
    int mode;
    bool pow = true;
    ...
    {
	    /* Amp Power on again */
	    tfa9887_power(pow, offload);
	    mode = select_profile(dev->current_mode);
	    tfa9887_set_mode(mode);
	    /* Change amp path according to offload */
	    tfa9887_change_path(offload);
    }
    tfa9887_check_status();
...
amplifier_module_t HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .name = "ASUS ZE60xKL MSM8939 audio amplifier HAL",
